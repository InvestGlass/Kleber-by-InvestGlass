...
class _XPortfoliItemBarChartState extends State<XPortfoliItemBarChart> {
  ...
  Widget buildTrackBall(BuildContext context, TrackballDetails trackballDetails) {
    ...
    final amountString = amountValue == null ? '' : CommonFunctions.formatDoubleWithThousandSeperator('$amountValue', amountValue == 0, 2);
    return Container(
      // height: 50,
      ...
      child: Row(
        ...
        children: [
          Container(
            ...
            width: markerSize,
            decoration: BoxDecoration(
              color: seriesIndex == 0 ? Color(0xFFC8C8C8) : AppColors.kViolate,
  ...
  CartesianChartAnnotation buildSingleAnnotation(String x, double y, ChartAlignment horizontalAlignment) {
    final additionValue = getYAxisInterval() / 3;
    final value = double.tryParse('${y}') ?? 0;
    final valueStr = value == value.roundToDouble() ? '${value.round()}' : '$value';
        ...
        children: [
          ...
          if (value != 0)
            Icon(
              ...
              color: value > 0 ? FlutterFlowTheme.of(context).customColor2 : FlutterFlowTheme.of(context).customColor3,
              size: 18,
            ),
        ],
      ),
  ...
  List<CartesianChartAnnotation> getAnnotations() {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (showSecondColumn) {
        annotations.add(buildSingleAnnotation(x, widget.listY1[i], ChartAlignment.far));
        annotations.add(buildSingleAnnotation(x, widget.listY2[i], ChartAlignment.near));
      } else {
        annotations.add(buildSingleAnnotation(x, widget.listY1[i], ChartAlignment.center));
      }
    }
    return annotations;
  }
    ...
    return <CartesianSeries>[
      ...
      if (checkShowSecondColumn())
        ColumnSeries<String, String>(
          ...
          pointColorMapper: (String label, int index) => AppColors.kViolate,
          width: 0.8,
          name: 'ColumnSeries',
  ...
  double calculateWidth() {
    ...
    return barGroupSpace * (length) + groupsSpace * (length - 1) + leftTitleReservedSize + bonusSpace;
  }
